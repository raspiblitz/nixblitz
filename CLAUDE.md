...
- `nix/wasmtime.nix` pins wasmtime to a specific version (46.0.1) via the official bytecodealliance c-api release, INDEPENDENT of every nixpkgs input. This is not a flake input but the same principle: the `wasmtime_dart` FFI bindings are transcribed from a specific wasmtime major's C headers, so the runtime `libwasmtime.so` cannot float with whatever nixpkgs a node happens to build with — a mismatch is silent ABI drift. bitcoind is the opposite case (subprocess with a stable CLI → floats freely). Both the wrapper's baked lib and `gen-wasmtime-bindings`' headers come from this one pinned derivation, so they move together by construction. Bump = edit `version` + the two SRI hashes in `nix/wasmtime.nix`, then `just gen-wasmtime-bindings`.

Whenever a third input exception is genuinely warranted, add it here AND in a comment above the input.

### Adding a Dart workspace member

When you add a new package to the root `pubspec.yaml` `workspace:` list (as `wasmtime_dart` was for the WASM sandbox), you MUST also add it in THREE places in `nix/tui_pkg.nix` or the Nix build of the TUI won't see it (tests pass via the live pub workspace, but `nix build .#nixblitz-unwrapped` fails with "X isn't defined"): the `nixFilter` `include` list (source into the sandbox), `workspaceMembers`, and the slimmed `postPatch` workspace pubspec. Then `just gen-locks`.
